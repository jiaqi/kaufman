...
public class ChainedCredentialsProvider implements AWSCredentialsProvider {
  ...
  public ChainedCredentialsProvider(List<Object> chain) throws IOException {
    for (Object initObject : chain) {
      ...
      if (initObject instanceof AWSCredentials) {
        ...
        instance = new StaticCredentialsProvider((AWSCredentials) initObject);
        return;
      }
      if (initObject instanceof File || initObject instanceof String) {
        File file;
        if (initObject instanceof String) {
          file = new File(ExpressionUtils.populate(((String) initObject).trim()));
        } else {
        ...
        instance = new StaticCredentialsProvider(new PropertiesCredentials(file));
        return;
      }
    }
