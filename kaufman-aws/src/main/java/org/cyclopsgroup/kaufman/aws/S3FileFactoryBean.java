...
public class S3FileFactoryBean implements FactoryBean<File>, DisposableBean {
  ...
  public File getObject() throws IOException {
    if (localFile == null) {
      localFile = File.createTempFile(S3FileFactoryBean.class.getSimpleName(), ".tmp");
      LOG.info("Using default local file " + localFile);
    }
    LOG.info(
        String.format(
            "Getting object %s:%s into local file %s",
            bucketName, objectKey, localFile.getAbsolutePath()));
    ObjectMetadata result =
        s3Client.getObject(new GetObjectRequest(bucketName, objectKey), localFile);
