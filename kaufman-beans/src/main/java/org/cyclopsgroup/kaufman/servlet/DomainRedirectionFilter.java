...
/**
 * Filter that redirects requests to the primary domain if request is not from it. For example, it
 * can be configured to redirect http://mysite.com to http://www.mysite.com, or from
...
public class DomainRedirectionFilter implements Filter {
  ...
  private static class ForwardedHttpRequest extends HttpServletRequestWrapper {
    ...
    protected ForwardedHttpRequest(HttpServletRequest request) {
      ...
      if (request.getHeader(HEADER_X_FORWARDED_PORT) != null) {
        this.serverPort = Integer.parseInt(request.getHeader(HEADER_X_FORWARDED_PORT));
        changed = true;
      } else {
    ...
    public StringBuffer getRequestURL() {
      ...
      String originalBase =
          buildUrl(super.getScheme(), super.getServerName(), super.getServerPort());
      String url = super.getRequestURL().toString();
      if (url.startsWith(originalBase)) {
        String newBase = buildUrl(scheme, super.getServerName(), serverPort);
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    HttpServletRequest req = new ForwardedHttpRequest((HttpServletRequest) request);
    ...
    String requestBase = buildUrl(req.getScheme(), req.getServerName(), req.getServerPort());
    ...
    String redirectUrl = baseUrl + requestedUrl.substring(requestBase.length());
    LOG.info("Redirecting request from " + requestedUrl + " to new location: " + redirectUrl);
