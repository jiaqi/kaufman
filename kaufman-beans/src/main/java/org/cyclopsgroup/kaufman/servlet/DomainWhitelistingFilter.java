...
public class DomainWhitelistingFilter extends AbstractRequestAuthorizingFilter {
  ...
  private final Set<String> buildinDomains =
      Collections.unmodifiableSet(new HashSet<String>(Arrays.asList("127.0.0.1", "localhost")));
  ...
  public void init(FilterConfig config) throws ServletException {
    ...
    if (StringUtils.isNotBlank(names)) {
      whiteDomainNames =
          Collections.unmodifiableSet(
              new HashSet<String>(Arrays.asList(StringUtils.split(names, ','))));
    ...
    if (StringUtils.isNotBlank(hashes)) {
      whiteDomainHashes =
          Collections.unmodifiableSet(
              new HashSet<String>(Arrays.asList(StringUtils.split(hashes, ','))));
  ...
  protected boolean isRequestAuthorized(HttpServletRequest request)
    ...
    if (buildinDomains.contains(serverName) || whiteDomainNames.contains(serverName)) {
      return true;
    }
