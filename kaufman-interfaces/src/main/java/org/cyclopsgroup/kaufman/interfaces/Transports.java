...
public final class Transports {
  public static <T> List<T> transportList(
      List<? extends Transportable<T>> sources, Class<T> destinationType) {
    if (sources.isEmpty()) {
    ...
    for (Transportable<T> source : sources) {
      ...
      try {
        ...
        throw new IllegalStateException(
            "Can't instantiate destination type " + destinationType + ": " + e.getMessage(), e);
