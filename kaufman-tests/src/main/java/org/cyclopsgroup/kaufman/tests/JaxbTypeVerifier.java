  ...
  private void verifyXmlElement(Class<?> elementType) {
    try {
      for (PropertyDescriptor prop :
          Introspector.getBeanInfo(elementType).getPropertyDescriptors()) {
        ...
        if (prop.getReadMethod().isAnnotationPresent(XmlElement.class)
          ...
          assertNotNull(
              "Property " + prop.getName() + " in " + elementType + " is missing setter", setter);

          assertFalse(
              "Setter of " + prop.getName() + " is not supposed to be annotated in " + elementType,
              setter.isAnnotationPresent(XmlElement.class)
                  || setter.isAnnotationPresent(XmlAttribute.class));
          if (options.verifyDateTimeAdapter && prop.getPropertyType() == DateTime.class) {}
    ...
    for (Enum<T> enu : EnumSet.allOf(enumType)) {
      ...
      try {
        ...
        throw new IllegalStateException(
            "Can't get field " + enu.name() + " of enum " + enumType, e);
      }
      XmlEnumValue value = field.getAnnotation(XmlEnumValue.class);
      assertNotNull(
          enumType + "." + enu.name() + " is not annotated with " + XmlEnumValue.class, value);
